...
best_model_file_path = 'EdgeGAT_binary_best_model.pth'

# 定义分类报告文件路径
report_file_path = 'EdgeGAT_binary_classification_report_best.json'
...
for epoch in tqdm(range(1, epochs + 1), desc="Training Epochs"):
    ...
    # 计算当前模型的 accuracy，如果高于最高的 accuracy，则保存模型
    # current_accuracy = compute_accuracy(pred[train_mask], edge_label[train_mask])
    # if current_accuracy > best_accuracy:
    #     best_accuracy = current_accuracy
    #     th.save(model, 'EdgeGAT_binary_best_model.pth')
    #     print(f'New best model saved at epoch {epoch} with accuracy: {best_accuracy}')
